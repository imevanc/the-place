...
const footerLinks: Array<{ href: string; icon: ReactElement; name: string }> = [
  { href: 'https://www.facebook.com', icon: <FaFacebook />, name: 'Facebook' },
  { href: 'https://www.instagram.com', icon: <FaInstagram />, name: 'Instagram' },
  { href: 'https://www.x.com', icon: <FaXTwitter />, name: 'Twitter' },
];
  ...
  return (
    <footer className="w-full bg-primary-300 relative bottom-0 py-4 px-4">
      <div className="w-full flex flex-col md:flex-row justify-between items-center">
        ...
        <div className="md:w-[10%] flex justify-between items-center mb-4 md:mb-0 order-first md:order-last">
          {footerLinks.map(({ icon, href, name }, idx) => (
            <div key={href + idx} >
              <Link
                href={href}
                target="_blank"
